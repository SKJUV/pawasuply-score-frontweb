...
import type { DepositResponse, PayoutResponse } from '@/lib/types';
import Spinner from '@/components/Spinner';
import WalletLimitAlert from '@/components/WalletLimitAlert';
import SectionHeader from '@/components/SectionHeader';
import { IconCheck, IconX, IconSend, IconWarning, IconPhone } from '@/components/icons';

// ── Fixtures sandbox pawaPay ──────────────────────────────────────────────────
...
// Suffixes sandbox (section 9 du spec)
const PH_OK       = '237653456789'; // COMPLETED
const PH_INSUF    = '237653456049'; // INSUFFICIENT_BALANCE
const PH_DECLINED = '237653456039'; // PAYMENT_NOT_APPROVED
...
type Variant = 'green' | 'red' | 'violet' | 'orange' | 'zinc';
...
interface ActionDef {
  ...
  run:         (amount: number) => Promise<ResultState>;
}
...
function SimulatorContent() {
  ...
  const actions: ActionDef[] = [
    // ── Dépôts ────────────────────────────────────────────────────────────
    {
      ...
      run: (amount) => makeDeposit(PH_OK, DEMO_A)(amount),
    },
    {
      id: 'deposit-insuf', label: 'Solde insuffisant', category: 'Dépôts Mobile Money',
      description: 'INSUFFICIENT_BALANCE → fréquence pénalisée (suffixe 049)',
      variant: 'red', Icon: IconX,
      run: (amount) => makeDeposit(PH_INSUF, DEMO_A)(amount),
    },
    {
      id: 'deposit-declined', label: 'PIN refusé', category: 'Dépôts Mobile Money',
      description: 'PAYMENT_NOT_APPROVED → PIN non saisi / expiré (suffixe 039)',
      variant: 'red', Icon: IconX,
      run: (amount) => makeDeposit(PH_DECLINED, DEMO_A)(amount),
    },
    {
      id: 'deposit-notfound', label: 'Numéro inconnu', category: 'Dépôts Mobile Money',
      description: 'PAYER_NOT_FOUND → numéro non enregistré chez MTN (suffixe 029)',
      variant: 'zinc', Icon: IconPhone,
      run: (amount) => makeDeposit(PH_NOTFOUND, DEMO_A)(amount),
    },
    // ── Payouts ───────────────────────────────────────────────────────────
    {
    ...
    {
      id: 'payout-wallet-limit', label: 'Limite portefeuille', category: 'Payouts Crédit Stock',
      description: 'Simule WALLET_LIMIT_REACHED → crédit CANCELLED, aucune pénalité score (suffixe 099)',
  ...
  async function run(action: ActionDef) {
    ...
    setResult(null);
    try {
      setResult(await action.run(customAmount));
    } catch (e) {
        ...
        <div className="flex flex-wrap gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              className="w-44 rounded-xl border border-violet-200 bg-violet-50/40 px-3 py-2.5 text-sm tabular-nums outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100"
            />
          </div>
        </div>
      </div>
        ...
        <div className="card p-5">
          ...
          {result.type === 'deposit' && (
            <div className={`rounded-xl border p-5 ${
              result.data.status === 'ACCEPTED' ? 'border-emerald-200 bg-emerald-50' : 'border-red-100 bg-red-50'
            }`}>
              ...
              {result.data.status === 'ACCEPTED' && (
                <div className="mt-3 space-y-2">
                  <p className="rounded-lg bg-emerald-100 px-3 py-2 text-xs text-emerald-600">
                    Statut final via webhook pawaPay · Écoutez <code className="font-bold">score:updated</code> sur Socket.io
                  </p>
                  <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2">
                    <p className="text-xs font-semibold text-amber-800">Pop-up PIN non reçu ?</p>
                    <p className="mt-0.5 text-xs text-amber-700">Composez le code USSD manuellement sur MTN :</p>
                    <a
                      href="tel:*126#"
          ...
          {result.type !== 'error' && result.type !== 'walletLimit' && (
            <details className="mt-4">
              ...
              <pre className="mt-2 overflow-x-auto rounded-xl bg-zinc-900 p-4 text-xs leading-relaxed text-zinc-100">
                {JSON.stringify(result.type === 'deposit' ? result.data : result.data, null, 2)}
              </pre>
            </details>
          )}
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="card p-5">
          ...
          <div className="space-y-2 text-xs">
            {[
              { key: 'A', id: DEMO_A, info: 'Amadou Diallo · Platine — dépôts' },
              { key: 'B', id: DEMO_B, info: 'Boutiquier B — payouts grossiste' },
            ].map(({ key, id, info }) => (
              <div key={key} className="rounded-lg bg-violet-50 p-3">
                <span className="font-bold text-violet-700">Boutiquier {key}</span>
                <span className="ml-1.5 text-zinc-500">{info}</span>
